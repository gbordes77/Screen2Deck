...
class BenchmarkResult:
    ...
    def passed(self) -> bool:
        ...
        return (
            self.exact_accuracy >= 0.95
            and self.total_time_ms < 5000
...
class E2EBenchRunner:
    ...
    async def upload_image(self, image_path: Path) -> Tuple[str, float]:
        ...
        with open(image_path, "rb") as f:
            ...
            data.add_field(
                "file", f, filename=image_path.name, content_type="image/jpeg"
            )

            async with self.session.post(
                f"{self.api_url}/api/ocr/upload", data=data
            ) as resp:
    ...
    async def run_benchmark(
        self, images_dir: Path, golden_dir: Path
        ...
        for image_path in image_files:
            ...
            print(
                f"  {status} Accuracy: {result.exact_accuracy:.1%} | Time: {result.total_time_ms:.0f}ms"
            )
    ...
    def generate_report(self, results: List[BenchmarkResult]) -> Dict:
        ...
        avg_accuracy = (
            sum(r.exact_accuracy for r in results) / total_images
            if total_images > 0
            else 0
        )
        avg_lenient = (
            sum(r.lenient_accuracy for r in results) / total_images
            if total_images > 0
            else 0
        )
        avg_time = (
            sum(r.total_time_ms for r in results) / total_images
            if total_images > 0
            else 0
        )
        p95_time = (
            sorted([r.total_time_ms for r in results])[int(total_images * 0.95)]
            if total_images > 0
            else 0
        )
            ...
            "acceptance": {
                ...
                "passed": (
                    (
                        avg_accuracy >= 0.95
                        and p95_time <= 5000
                        and passed_images / total_images >= 0.8
                    )
                    if total_images > 0
                    else False
                ),
    ...
    def print_report(self, report: Dict):
        ...
        print(
            f"  - Target Accuracy: ≥{acceptance['target_accuracy']:.0%} (got {summary['avg_exact_accuracy']:.1%})"
        )
        print(
            f"  - Target P95 Time: ≤{acceptance['target_p95_ms']}ms (got {summary['p95_time_ms']:.0f}ms)"
        )
        print(
            f"  - Target Pass Rate: ≥{acceptance['target_pass_rate']:.0%} (got {summary['pass_rate']:.1%})"
        )
        ...
        for r in report["details"]:
            details_data.append(
                [
                    ...
                    (
                        "✅"
                        if r["exact_accuracy"] >= 0.95 and r["total_time_ms"] < 5000
                        else "❌"
                    ),
                ]
            )
...
async def main():
    ...
    parser.add_argument(
        "--images",
        type=Path,
        default="./decklist-validation-set",
        help="Directory containing test images",
    )
    parser.add_argument(
        "--golden",
        type=Path,
        default="./validation_set/golden",
        help="Directory containing golden JSON files",
    )
    parser.add_argument(
        "--out", type=Path, default="./reports", help="Output directory for reports"
    )
    parser.add_argument(
    ...
    async with E2EBenchRunner(args.api_url) as runner:
        ...
        with open(md_path, "w") as f:
            ...
            f.write(
                f"- **Avg Accuracy**: {report['summary']['avg_exact_accuracy']:.1%}\n"
            )
            f.write(f"- **P95 Time**: {report['summary']['p95_time_ms']:.0f}ms\n")
            f.write(
                f"- **Acceptance**: {'✅ PASSED' if report['acceptance']['passed'] else '❌ FAILED'}\n"
            )
