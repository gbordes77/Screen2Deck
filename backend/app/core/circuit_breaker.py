...
class VisionFallbackCircuitBreaker:
    ...
    def should_use_fallback(self, confidence: float, lines: int) -> bool:
        ...
        if self.state == CircuitState.OPEN:
            ...
            if (
                ...
                and time.time() - self.circuit_opened_at > self.recovery_timeout
            ):
    ...
    def record_failure(self, error: Exception) -> None:
        ...
        logger.error(
            f"Vision API failure {self.failure_count}/{self.failure_threshold}: {error}"
        )
    ...
    def _adjust_thresholds(self) -> None:
        ...
        self.current_confidence_threshold = min(
            0.95, self.current_confidence_threshold + 0.05
        )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_adjustment": (
                self.threshold_adjustments[-1] if self.threshold_adjustments else None
            ),
        }
    ...
    def get_thresholds(cls, width: int, height: int) -> Dict[str, Any]:
        ...
        for band_name, band_config in cls.RESOLUTION_BANDS.items():
            if pixels <= band_config["max_pixels"]:
                logger.debug(
                    f"Using {band_name} resolution thresholds for {width}x{height}"
                )
                return {
