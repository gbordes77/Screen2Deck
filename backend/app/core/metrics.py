...
ocr_confidence_mean = Gauge(
    "screen2deck_ocr_confidence_mean", "Mean OCR confidence score", registry=REGISTRY
)
...
cache_hit_ratio = Gauge(
    "screen2deck_cache_hit_ratio", "Cache hit ratio", ["cache_type"], registry=REGISTRY
)
...
active_jobs = Gauge(
    "screen2deck_active_jobs", "Number of active OCR jobs", registry=REGISTRY
)

job_queue_size = Gauge(
    "screen2deck_job_queue_size", "Size of job queue", registry=REGISTRY
)
...
def track_ocr_request(cached: bool = False, status: str = "success"):
    ...
    ocr_requests_total.labels(status=status, cached="true" if cached else "false").inc()
...
def track_auth_attempt(method: str, success: bool):
    ...
    auth_attempts_total.labels(
        method=method, status="success" if success else "failure"
    ).inc()
