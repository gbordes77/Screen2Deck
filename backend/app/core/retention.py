...
# Initialize Celery
celery_app = Celery(
    "retention", broker=str(settings.REDIS_URL), backend=str(settings.REDIS_URL)
)
    ...
    try:
            ...
            for image_file in upload_dir.glob("*.{jpg,jpeg,png,gif,bmp}"):
                try:
                    ...
                    if file_age < cutoff:
                        if dry_run:
                            logger.info(
                                f"[DRY RUN] Would delete expired image: {image_file.name}"
                            )
                        else:
                            image_file.unlink()
                            retention_deleted_total.labels(
                                type="images", reason="expired"
                            ).inc()
                        stats["deleted_files"] += 1
                        logger.info(
                            f"{'[DRY RUN] Would delete' if dry_run else 'Deleted'} expired image: {image_file.name}"
                        )
        ...
        for key in redis_client.scan_iter(match="image:*"):
            try:
                ...
                if ttl == -1:  # No TTL set
                    redis_client.expire(
                        key, int(RETENTION_PERIODS["images"].total_seconds())
                    )
    ...
    try:
        ...
        for key in redis_client.scan_iter(match="hash:*"):
            try:
                ...
                if ttl == -1:  # No TTL set
                    redis_client.expire(
                        key, int(RETENTION_PERIODS["hashes"].total_seconds())
                    )
        ...
        for key in redis_client.scan_iter(match=f"hash:*:user:{user_id}"):
            ...
            if hash_data:
                user_data["images"].append(
                    {"hash": key.decode().split(":")[1], "data": hash_data.decode()}
                )
