...
class JobStorage:
        ...
        if created:
            ...
            if image_hash:
                await self.redis_client.sadd(
                    self._index_key("hash", image_hash), job_id
                )
                await self.redis_client.expire(
                    self._index_key("hash", image_hash), int(self.ttl.total_seconds())
                )
            ...
            if user_id:
                ...
                await self.redis_client.expire(
                    self._index_key("user", user_id), int(self.ttl.total_seconds())
                )
    ...
    async def update_job(
        ...
        await self.redis_client.set(
            key, json.dumps(job_data), ex=int(self.ttl.total_seconds())
        )
    ...
    async def find_by_image_hash(self, image_hash: str) -> Optional[str]:
        ...
        job_ids = await self.redis_client.smembers(self._index_key("hash", image_hash))
    ...
    async def get_user_jobs(
        ...
        job_ids = await self.redis_client.zrevrange(
            self._index_key("user", user_id), offset, offset + limit - 1
        )
...
async def set_job(job_id: str, data: Dict[str, Any]) -> bool:
    ...
    return await job_storage.update_job(job_id, state=state, result=result, error=error)
