...
class ConnectionManager:
    ...
    async def connect(
        self, websocket: WebSocket, job_id: str, user_id: Optional[str] = None
    ):
    ...
    async def get_job_status(self, job_id: str) -> dict:
        ...
        if not status:
            status = {"state": "unknown", "progress": 0, "message": "Job not found"}
        return status
        ...
        while True:
            ...
            if status.get("state") in ["completed", "failed", "cancelled"]:
                await websocket.send_json(
                    {"type": "close", "message": f"Job {status['state']}"}
                )
                break
...
async def notify_job_update(
    job_id: str, state: str, progress: int, result: Optional[dict] = None
):
