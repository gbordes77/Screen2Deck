...
class Settings:
    ...
    OCR_EARLY_STOP_CONF: float = float(
        os.getenv("OCR_EARLY_STOP_CONF", 0.85)
    )  # Early termination threshold
    OCR_MIN_SPAN_CONF: float = float(
        os.getenv("OCR_MIN_SPAN_CONF", 0.3)
    )  # Min confidence per text span
    SUPERRES_MIN_WIDTH: int = int(
        os.getenv("SUPERRES_MIN_WIDTH", 1200)
    )  # Min width to trigger super-resolution
    ...
    ALWAYS_VERIFY_SCRYFALL: bool = (
        os.getenv("ALWAYS_VERIFY_SCRYFALL", "true").lower() == "true"
    )
    ENABLE_SCRYFALL_ONLINE_FALLBACK: bool = (
        os.getenv("ENABLE_SCRYFALL_ONLINE_FALLBACK", "true").lower() == "true"
    )
    ...
    SCRYFALL_BULK_PATH: str = os.getenv(
    ...
    HEALTH_EXPOSE_INTERNAL: bool = (
        os.getenv("HEALTH_EXPOSE_INTERNAL", "false").lower() == "true"
    )
    HEALTH_ALLOWED_IPS: str = os.getenv(
        "HEALTH_ALLOWED_IPS", "127.0.0.1,::1"
    )  # Comma-separated
    HEALTH_REQUIRE_AUTH: bool = (
        os.getenv("HEALTH_REQUIRE_AUTH", "true").lower() == "true"
    )
