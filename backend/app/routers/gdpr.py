...
async def delete_data(identifier: str) -> Dict[str, Any]:
    ...
    """
    try:
        deleted_items = {"job": False, "result": False, "image": False, "hash": False}
        ...
        # Check if it's an image hash (SHA256)
            ...
            if keys_deleted > 0:
                ...
                retention_deleted_total.labels(
                    type="hashes", reason="gdpr_request"
                ).inc()
                logger.info(
                    f"GDPR deletion: {keys_deleted} keys for hash {identifier} deleted"
                )
